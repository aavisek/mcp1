...
- The list of Azure MCP Server tool areas (namespaces) you wish to enable (see [azmcp-commands.md](https://github.com/microsoft/mcp/blob/main/servers/Azure.Mcp.Server/docs/azmcp-commands.md)). This reference template defaults to the `storage` namespace, but for Azure Virtual Desktop use `virtualdesktop` (or both). Example: `['storage','virtualdesktop']`.
...
- **Container App** - Runs Azure MCP Server with selected namespace(s), such as `storage`, `virtualdesktop`, or both.
- **User-assigned managed identity** - A user-assigned managed identity with the configured role (default: Subscription Reader) for the subscription the resources are deployed to. This managed identity will be assigned to the container app and used by the Azure MCP server to make tool calls.
